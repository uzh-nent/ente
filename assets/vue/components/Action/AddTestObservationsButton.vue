<template>
  <button-confirm-modal
      :title="$t('_action.add_test_observations.title')" icon="fas fa-plus"
    ...
    <p v-if="missingAnalysisTypes.length === 0">
      {{ $t('_action.add_test_observations.no_missing_observations') }}
    </p>
    <template v-else>
      ...
      <div v-for="analysisType in missingAnalysisTypes" :key="analysisType" class="mt-3 p-2 bg-light">
        <checkbox
            ...
            :model-value="observationEnabled(analysisType)" @update:modelValue="toggleObservation(analysisType, $event)"
        />
        <test-form class="mt-3" v-if="observationEnabled(analysisType)"
                   :id="analysisType" @update="updateObservation(analysisType, $event)"/>
      </div>
    </template>
  </button-confirm-modal>
</template>
...
export default {
  ...
  methods: {
    ...
    confirm: async function () {
      ...
      const successMessage = this.$t('_action.add_test_observations.added')
